...
fn validation_error_message(error: &ValidationError) -> &'static str {
    match error {
        ...
        ValidationError::InvalidFullName => {
            "Full name must be 3-100 characters and contain only letters, numbers and spaces"
        }
        ValidationError::InvalidPhone => "Invalid phone format",
        ValidationError::WeakPassword => {
            "Password must be at least 8 characters and contain uppercase letter, lowercase letter and digit"
        }
...
pub fn validation_error_to_response(errors: Vec<ValidationError>) -> HttpResponse {
    let messages: Vec<&str> = errors.iter().map(validation_error_message).collect();
